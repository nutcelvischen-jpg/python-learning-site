...
以下是赫哥用**模擬資料**跑出來的範例文字雲（**不是真實 PTT 抓的**，純教學示範）：

![PTT 空汙文字雲範例](../assets/images/chart31_ptt_air_wordcloud_example.png)

觀察重點：

- **大字關鍵字** = 詞頻最高 = 鄉民最常討論：`火力發電廠`、`肺癌`、`空氣品質`、`口罩`、`咳嗽`
- **配色** = `viridis` colormap（綠→紫漸層），詞頻越高顏色越深
- **jieba 切詞盲點**：注意「火力」跟「發電廠」分開、「空氣」跟「品質」分開 — 這是真實 jieba 行為，要讓它們當一個詞需要 `jieba.add_word()` 客製化詞典
- 學生自己跑會看到**不一樣的關鍵字**（依當下 PTT 熱門議題浮動）— 這是文字雲有趣的地方
